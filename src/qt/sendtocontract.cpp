    ...
    m_results(1)
{
    ...
    ui->labelSenderAddress->setToolTip(tr("The vuicash address that will be used as sender."));
...
void SendToContract::setModel(WalletModel *_model)
{
    ...
    // update the display unit, to not use the default ("VUI")
    updateDisplayUnit();
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonSendToContract->setToolTip(tr("Creates a Partially Signed VuiCash Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
...
void SendToContract::on_sendToContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed VuiCash Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
