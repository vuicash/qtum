...
class VuiCashHwiToolPriv
{
public:
    VuiCashHwiToolPriv(QObject *parent)
    {
...
VuiCashHwiTool::VuiCashHwiTool(QObject *parent) : QObject(parent)
{
    d = new VuiCashHwiToolPriv(this);
}

VuiCashHwiTool::~VuiCashHwiTool()
{
    delete d;
}

bool VuiCashHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(VuiCashLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(VuiCashLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool VuiCashHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = VuiCashLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(VuiCashLedger::instance().errorMessage());
    return ret;
}

bool VuiCashHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = VuiCashLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool VuiCashHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool VuiCashHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool VuiCashHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool VuiCashHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool VuiCashHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = VuiCashLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(VuiCashLedger::instance().errorMessage());
    return ret;
}

bool VuiCashHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = VuiCashLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(VuiCashLedger::instance().errorMessage());
    return ret;
}

bool VuiCashHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString VuiCashHwiTool::errorMessage()
{
...
bool VuiCashHwiTool::isStarted()
{
    return d->fStarted;
}

void VuiCashHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool VuiCashHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool VuiCashHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool VuiCashHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool VuiCashHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool VuiCashHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool VuiCashHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void VuiCashHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool VuiCashHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void VuiCashHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString VuiCashHwiTool::derivationPathPKH()
{
    std::string path = VuiCashLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString VuiCashHwiTool::derivationPathP2SH()
{
    std::string path = VuiCashLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString VuiCashHwiTool::derivationPathBech32()
{
    std::string path = VuiCashLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
