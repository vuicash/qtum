...
ResultExecute VuiCashState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, VuiCashTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, VuiCashTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, VuiCashTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}
...
void VuiCashState::validateTransfersWithChangeLog(){
...
void VuiCashState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!VuiCashState::addressInUse(delegationsAddress)){
        VuiCashState::createContract(delegationsAddress);
        VuiCashState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, VuiCashState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
